...
def import_data(pathdata):
    df_raw = pd.read_excel(os.path.join(pathdata, 'data.xlsx'), engine='openpyxl')
    try:
...
def fit_and_saveprms(datax, datay, thisarea, model, title, figtitle, save_path, y1, y2, isexp, logscale):
    ...
    if isexp == 1:
        ...
        if model.name == 'Model(growth)':
            pars = model.make_params(i_max = np.max(datay), ks = datax[hfn.find_nearest(datay, np.max(datay)/2)[0]])
        elif model.name == 'Model(hill)':
            pars = model.make_params(i_max = np.max(datay), ks = datax[hfn.find_nearest(datay, np.max(datay)/2)[0]], n = 0.5)
            pars['ks'].set( min=0.0001)
            pars['i_max'].set( min=np.min(datay))
        else:
